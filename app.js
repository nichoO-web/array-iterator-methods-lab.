...
const veryOldInventors = inventors.filter((element) => {
    return element.year < 1600 && element.year >= 1500;
});
...
const sortedByBirthYear = inventors.sort((a, b) => (a.year > b.year) ? 1 : (a.year < b.year) ? -1 : 0);
...
const firstLast = people.map((element) => {
...
const isEveryone19OrOlder = devs.every((ages) => {
  return ages.year <= 2005;
})
...
const idx = comments.findIndex((commentidx) => {
  return commentidx.id === 123523;
})
